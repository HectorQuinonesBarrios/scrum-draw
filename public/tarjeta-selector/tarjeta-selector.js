...
riot.tag2('tarjeta-selector', '<div class="row" id="sortableKanbanBoards"> <div class="panel panel-primary kanban-col" each="{backlog, i in backlogs}"> <div class="panel-heading">{backlog.tipo}<i class="fa fa-2x fa-plus-circle pull-right"></i></div> <div class="panel-body"> <div class="kanban-centered" id="{backlog._id}"> <article class="kanban-entry grab" id="item6" draggable="true" each="{tarjeta, i in backlog.tarjetas}"> <div class="kanban-entry-inner"> <div class="kanban-label"> <h2>Tarea con valor de {tarjeta.valor}</h2> <p>Como : {tarjeta.narrativa.como}</p> <p>Quiere : {tarjeta.narrativa.quiero}</p> <p>De Manera Que : {tarjeta.narrativa.manera}</p> <p>Dado : {tarjeta.criterios.dado}</p> <p>Cuando : {tarjeta.criterios.cuando}</p> <p>Entonces : {tarjeta.criterios.entonces}</p> </div> </div> </article> </div> </div> <div class="panel-footer"> <button class="btn btn-info btn-sm" type="button" data-toggle="modal" data-target="#CardModal" data-backlog="{backlog._id}">Agregar tarjeta</button> </div> </div> </div> <div class="modal modal-static fade" id="processing-modal" role="dialog" aria-hidden="true"> <div class="modal-dialog"> <div class="modal-content"> <div class="modal-body"> <div class="text-center"><i class="fa fa-refresh fa-5x fa-spin"></i> <h4>Processing...</h4> </div> </div> </div> </div> </div>', '', '', function(opts) {
